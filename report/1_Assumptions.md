...
For develop this Music Ensemble Management System (MEMS), I made several assumptions base on the assignment requirement PDF.
...
- Each ensemble must have unique ID that auto-generated by system
...
- If no current ensemble is set, some commands like add musician will shows error message
...
- For Orchestra ensemble, there are 5 valid roles (instruments):
  1. Conductor
  2. Flute player
  3. Harpist
  4. Pianist
...
- All 12 functions in the system should implemented as separate Command classes
...
- CommandFactory is responsible for create all command objects
...
- Ensemble information display in format: "Type Name (ID)"
- Musician information display in format: "Name (ID), Role"
- List should be display in order they were added (maintain insertion order)
